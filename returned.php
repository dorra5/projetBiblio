...
if(isset($_GET['action'])){
     if($_GET['action']=='return')  { 
       ...
       $update= $db->prepare("UPDATE borrowing SET status = 'Returned', date = $date WHERE id = '$id'");
       $update->execute();
...
/*if(isset($_POST['return'])){
   ...
   echo $_POST['id']."aaaaaaaaaa";    
    
   $date = date("Y-m-d", strtotime("+8 HOURS"));
   $update= $db->prepare("UPDATE borrowing SET status = 'Returned', date = $date WHERE id = '$_POST[id]'");
   $update->execute();
    ...
    <div class="sidenav">
        <div id="opacity">
            ...
            <i class="fa fa-caret-down"></i>
                ...
                <div class="dropdown-container">
                <a href="borrowing.php"> <i class="fa fa-book" aria-hidden="true"></i> &nbsp;Books Details</a>
    ...
    <div class="main">
        ...
        <div id="admin_show_part" >
            ...
            <table id="myTable">
               ...
               <?php
                $selectBook=$db->prepare("SELECT * FROM borrowing ");
                $selectBook->execute();
                    while ($freturn=$selectBook->fetch()){
                        // echo $freturn['student_id'];
                  
               ?>	
                <tr>
                    <td>
                        <?php
                            $qstudent =$db->prepare("SELECT * FROM student WHERE id = '$freturn[student_id]'  ") ;
                            $qstudent->execute();
                            while ($fstudent=$qstudent->fetch()){
                    ...
                    <td>
                        <?php
                            $qbook = $db->prepare("SELECT * FROM book WHERE id = '$freturn[book_id]'") ;
                            $qbook->execute();
                            $fbook = $qbook->fetch();
                            echo $fbook['title'];
                        ?>
                    ...
                    <td>
                        <?php 
                            ...
                            echo "<center><center><label style='color:red;'>Returned</label></center></center>";
                                
                           
                            }else{
                                
                              echo '<center> <a href="?action=return&amp;id='.$freturn['id'].'"> <button type="button" class="borrowBook"> Return </button> </a> </center>'  ;
