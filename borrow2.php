...
?>
<?php
   /* $borrow = $db->prepare("SELECT SUM(qty) as total FROM borrowing "); 
    $borrow->execute();
    $new_qty0 = $borrow->fetch(PDO::FETCH_ASSOC);
    echo $new_qty0['total'];*/

  /*  $status="Borrowed";
    $borrow1 = $db->prepare("SELECT SUM(qty) as total FROM borrowing WHERE book_id = 1 and status = ' $status' "); 
    $borrow1->execute();
    $new_qty1 = $borrow1->fetch(PDO::FETCH_ASSOC);
    echo $new_qty1['total']. "aaaaa";*/
    ...
    if(!ISSET($_POST['student_id'])){	
        ...
        if(!ISSET($_POST['selector'])){
            ...
            foreach($_POST['selector'] as $key=>$value){
               ...
               // echo $book_id . "vvvvvvv";
                $student=$_POST['student_id'];
               // echo $student . "ddddddd";
                $qty = $value;
               // echo $qty;
...
<head>
    ...
    <style>
       ...
       /* Fixed sidenav, full height */
        ...
        #student1{margin:15px 80px 0px 120px;}
        #student{
            width: 80%;
            margin:15px 80px 8px 120px;
        }
...
<div id="opacity">
    ...
    <div class="main">
        ...
        <div id="msgEtat" >
           <p> Transaction / Borrowing </p>
        </div>
        <form method = "POST" action = "" enctype = "multipart/form-data">
        <div>
           <!-- <a href="?action=borrow"> <button class="addNew" ><i class="fa fa-plus" aria-hidden="true"></i>&nbsp;&nbsp;Borrow</button> </a> -->
           <button type="submit"  name ="borrow" class="addNew" ><i class="fa fa-plus" aria-hidden="true"></i>&nbsp;&nbsp;Borrow</button>
            <br />
            <label id = "student1">Student Name:</label>
            <br />
            <select name ="student_id"  id = "student">
                ...
                ?>
                  <option     value="<?php echo $f['id'] ?>"   >   <?php echo $f['firstname']." ".$f['lastname']?>        </option>
                    ...
                    <td>
                        <?php
                            if($left == 0){
                                ...
                                echo '<input type = "hidden" name = "book_id['.$i.']" value = "'.$s['id'].'"><center><input type = "checkbox" name = "selector['.$i.']" value = "1"></center>';
                            }		
						?>
